...
function showAmount($amount, $decimal = 4, $separate = true, $exceptZeros = false, $currencyFormat = true)
{
    if ($amount == 0) {
        $decimal = 4;
    } else {
...
function levelCommission($user, $amount, $trx)
{
    ...
    while ($i <= $level) {
        ...
        $referralBonusBadge = UserBadge::where('user_id', $user->id)
            ->whereNotNull('referral_bonus_boost')
            ->orderBy('sequence_number', 'DESC')
            ->first();

        if ($referralBonusBadge) {
            $commissionValue = $commissionValue + ($commissionValue * $referralBonusBadge->referral_bonus_boost / 100);
        }
