...
@section('content')
    <section class="blog py-120">
        <div class="container">
                            ...
                            <div class="blog-item__inner">
                                <div class="blog-item__thumb">
                                    <a href="{{ route('blog.details', $blog->slug) }}" class="blog-item__thumb-link">
                                        <img src="{{ frontendImage('blog', 'thumb_' . @$blog->data_values->image, '405x225') }}"
                                            class="fit-image" alt="">
                                    </a>
                                    <span class="blog-item__date before-shadow flex-center"> <span class="d-block">
                                            {{ showDateTime($blog->created_at, 'd') }}</span> <span
                                            class="fs-16 d-block w-100">{{ showDateTime($blog->created_at, 'M') }}</span></span>
                                </div>
                                <div class="blog-item__content">
                                    <h5 class="blog-item__title"><a href="{{ route('blog.details', $blog->slug) }}"
                                            class="blog-item__title-link border-effect">{{ __($blog->data_values->title) }}</a>
                                    </h5>
            ...
            <div class="mt-4 mt-md-5 pagination-wrapper">
                {{ paginateLinks($blogs) }}
            </div>
        </div>
    </section>
